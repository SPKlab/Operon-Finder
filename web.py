...
if genome_id_option == search:
        ...
        if genome_id is None and (
            ...
            if not organisms:
                ...
                a_gene_symbol = next(re.finditer(rb"(\S*)\tBLAST_UniProt_GN_(?:OrderedLocusNames|ORFNames)", decompress(curl_output(f"https://stringdb-static.org/download/protein.aliases.v11.5/{genome_organism_id}.protein.aliases.v11.5.txt.gz")))).groups()[0].decode()
                features = loads(curl_output( 'https://patricbrc.org/api/genome_feature' , '--data-raw', f'and(keyword(%22{genome_organism_id}%22),keyword(%22{a_gene_symbol}%22))&limit(1)'))
                if features:
